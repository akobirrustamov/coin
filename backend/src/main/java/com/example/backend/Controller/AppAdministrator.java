...
@CrossOrigin
@RequestMapping("/api/v1/app/rector")
public class AppAdministrator {
    ...
    private String generateJwtToken(Administrator administrator) {
        ...
        return Jwts.builder()
                ...
                .setExpiration(new Date(System.currentTimeMillis() + 1000 * 6000)) // 1000 * 6000 ms = 1 hour
                .addClaims(claims)
    ...
    public HttpEntity<?> getAdministratorByToken(@PathVariable String token) {
        try {
            String administratorId = Jwts.parserBuilder()
                    .setSigningKey(getSigningKey())
                    .build()
                    .parseClaimsJws(token)
                    .getBody()
                    .getSubject();
            Administrator administrator = administratorRepo.findById(Integer.valueOf(administratorId))
