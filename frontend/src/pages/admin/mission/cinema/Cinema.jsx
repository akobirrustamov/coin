...
function Cinema() {
  ...
  const [formCinemaData, setFormCinemaData] = useState({
    title: "",
    description: "",
    mainPhoto: null,
    photos: [],
  ...
  useEffect(() => {
    if (activeFilter === "all") {
      setFilteredData(cinemaData);
    } else if (activeFilter === "active") {
      setFilteredData(cinemaData.filter((item) => item.status));
    } else {
      setFilteredData(cinemaData.filter((item) => !item.status));
    }
  }, [cinemaData, activeFilter]);

  const validateForm = () => {
    const newErrors = {};
    if (!formCinemaData.title.trim())
      newErrors.title = "Sarlavha kiritilishi shart";
    if (!formCinemaData.description.trim())
      newErrors.description = "Tavsif kiritilishi shart";
    if (formCinemaData.seconds <= 0) newErrors.seconds = "Ijobiy son kiriting";
    if (formCinemaData.coinCount <= 0)
      newErrors.coinCount = "Ijobiy son kiriting";
    if (!formCinemaData.url.trim()) newErrors.url = "URL kiritilishi shart";
  ...
  const handleInputChange = (e) => {
    ...
    setFormCinemaData((prev) => ({ ...prev, [name]: value }));
    if (errors[name]) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formCinemaData.mainPhoto) {
        mainPhotoId = await uploadImage(formCinemaData.mainPhoto, "main");
      }
      // ✅ Aks holda tahrirlash holatida mavjud rasmni saqlaymiz
      else if (editingId) {
        const existing = cinemaData.find((item) => item.id === editingId);
      ...
      for (const img of formCinemaData.photos || []) {
        const id = await uploadImage(img, "additional");
        photoIds.push(id);
      }

      const payload = {
        title: formCinemaData.title,
        description: formCinemaData.description,
        mainPhoto: mainPhotoId,
        seconds: Number(formCinemaData.seconds),
        coinCount: Number(formCinemaData.coinCount),
        url: formCinemaData.url,
        status: formCinemaData.status,
        createdAt: editingId ? formCinemaData.createdAt : Date.now(),
      };

      const method = editingId ? "PUT" : "POST";
      const url = editingId
        ? `/api/v1/task-cinema/${editingId}`
        : "/api/v1/task-cinema";
  ...
  const fetchCinemaMission = async () => {
    setIsLoading(true);
    try {
      const response = await ApiCall(
        "/api/v1/task-cinema",
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex justify-end space-x-3">
                    ...
                    <button className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center">
                      <svg
                        className="w-4 h-4 mr-2"
                        fill="none"
                        stroke="currentColor"
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        type="number"
                        name="coinCount"
                        value={formCinemaData.coinCount}
                        onChange={handleInputChange}
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition-all ${
                          errors.coinCount
                            ? "border-red-500 bg-red-50"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <textarea
                      name="description"
                      value={formCinemaData.description}
                      onChange={handleInputChange}
                  ...
                  <div className="space-y-4">
                    {editingId && !formCinemaData.mainPhoto ? (
                        ...
                        <div className="overflow-hidden rounded-lg border-2 border-dashed border-gray-300 aspect-video">
                          <img
                            src={`${baseUrl}/api/v1/file/getFile/${
                              cinemaData.find((c) => c.id === editingId)
                                ?.mainPhoto.id
                            }`}
                            alt="Asosiy rasm"
