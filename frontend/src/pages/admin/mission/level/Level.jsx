...
function Level() {
  ...
  const [formLevelData, setFormLevelData] = useState({
    title: "",
    description: "",
    mainPhoto: null,
    photos: [],
  ...
  useEffect(() => {
    if (activeFilter === "all") {
      ...
      setFilteredData(levelData.filter((item) => item.status));
    } else {
      setFilteredData(levelData.filter((item) => !item.status));
    }
  }, [levelData, activeFilter]);

  const validateForm = () => {
    const newErrors = {};
    if (!formLevelData.title.trim())
      newErrors.title = "Sarlavha kiritilishi shart";
    if (!formLevelData.description.trim())
      newErrors.description = "Tavsif kiritilishi shart";
    if (formLevelData.seconds <= 0) newErrors.seconds = "Ijobiy son kiriting";
    if (formLevelData.coinCount <= 0)
      newErrors.coinCount = "Ijobiy son kiriting";
    if (!formLevelData.userLevel)
      newErrors.userLevel = "Daraja kiritilishi shart";
  ...
  const handleInputChange = (e) => {
    ...
    setFormLevelData((prev) => ({ ...prev, [name]: value }));
    if (errors[name]) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formLevelData.mainPhoto) {
        mainPhotoId = await uploadImage(formLevelData.mainPhoto, "main");
      }
      // ✅ Aks holda tahrirlash holatida mavjud rasmni saqlaymiz
      else if (editingId) {
        const existing = levelData.find((item) => item.id === editingId);
      ...
      for (const img of formLevelData.photos || []) {
        const id = await uploadImage(img, "additional");
        photoIds.push(id);
      }

      const payload = {
        title: formLevelData.title,
        description: formLevelData.description,
        mainPhoto: mainPhotoId,
        seconds: Number(formLevelData.seconds),
        coinCount: Number(formLevelData.coinCount),
        userLevel: formLevelData.userLevel,
        status: formLevelData.status,
        createdAt: editingId ? formLevelData.createdAt : Date.now(),
      };

      const method = editingId ? "PUT" : "POST";
      const url = editingId
        ? `/api/v1/task-level/${editingId}`
        : "/api/v1/task-level";
          ...
          <div className="bg-white rounded-lg shadow p-6 text-center">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900">
              {activeFilter === "all"
                ...
                : activeFilter === "active"
                ? "Faol darajalar mavjud emas"
                : "Nofaol darajalar mavjud emas"}
            </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {/* <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Daraja davomiyligi (soniya){" "}
                        <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="number"
                        name="seconds"
                        value={formLevelData.seconds}
                        onChange={handleInputChange}
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition-all ${
                          errors.seconds
                            ? "border-red-500 bg-red-50"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <textarea
                      name="description"
                      value={formLevelData.description}
                      onChange={handleInputChange}
                  ...
                  <div className="space-y-4">
                    {editingId && !formLevelData.mainPhoto ? (
                        ...
                        <div className="overflow-hidden rounded-lg border-2 border-dashed border-gray-300 aspect-video">
                          <img
                            src={`${baseUrl}/api/v1/file/getFile/${
                              levelData.find((c) => c.id === editingId)
                                ?.mainPhoto.id
                            }`}
                            alt="Asosiy rasm"
