...
function Special() {
  ...
  const [formSpecialData, setFormSpecialData] = useState({
    title: "",
    description: "",
    mainPhoto: null,
    seconds: 0,
  ...
  useEffect(() => {
    if (activeFilter === "all") {
      setFilteredData(specialData);
    } else if (activeFilter === "active") {
      setFilteredData(specialData.filter((item) => item.status));
    } else {
      setFilteredData(specialData.filter((item) => !item.status));
    }
  }, [specialData, activeFilter]);

  const validateForm = () => {
    const newErrors = {};
    if (!formSpecialData.title.trim())
      newErrors.title = "Sarlavha kiritilishi shart";
    if (!formSpecialData.description.trim())
      newErrors.description = "Tavsif kiritilishi shart";
    if (formSpecialData.seconds <= 0) newErrors.seconds = "Ijobiy son kiriting";
    if (formSpecialData.coinCount <= 0)
  ...
  const handleInputChange = (e) => {
    ...
    setFormSpecialData((prev) => ({ ...prev, [name]: value }));
    if (errors[name]) {
  ...
  const handleMainPhotoChange = (e) => {
    ...
    setFormSpecialData((prev) => ({ ...prev, mainPhoto: file }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formSpecialData.mainPhoto) {
        mainPhotoId = await uploadImage(formSpecialData.mainPhoto, "main");
      }
      // ✅ Aks holda tahrirlash holatida mavjud rasmni saqlaymiz
      else if (editingId) {
        const existing = specialData.find((item) => item.id === editingId);
      ...
      for (const img of formSpecialData.photos || []) {
        const id = await uploadImage(img, "additional");
        photoIds.push(id);
      }

      const payload = {
        title: formSpecialData.title,
        description: formSpecialData.description,
        mainPhoto: mainPhotoId,
        seconds: Number(formSpecialData.seconds),
        coinCount: Number(formSpecialData.coinCount),
        status: formSpecialData.status,
        createdAt: editingId ? formSpecialData.createdAt : Date.now(),
      };

      const method = editingId ? "PUT" : "POST";
      const url = editingId
        ? `/api/v1/task-special/${editingId}`
        : "/api/v1/task-special";
  ...
  const fetchSpecialMission = async () => {
    setIsLoading(true);
    try {
      const response = await ApiCall(
        "/api/v1/task-special",
          ...
          <div className="bg-white rounded-lg shadow p-6 text-center">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900">
              {activeFilter === "all"
                ...
                : activeFilter === "active"
                ? "Faol topshiriq mavjud emas"
                : "Nofaol topshiriq mavjud emas"}
            </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        type="number"
                        name="coinCount"
                        value={formSpecialData.coinCount}
                        onChange={handleInputChange}
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition-all ${
                          errors.coinCount
                            ? "border-red-500 bg-red-50"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Topshriq davomiyligi (soniya){" "}
                        <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="number"
                        name="seconds"
                        value={formSpecialData.seconds}
                        onChange={handleInputChange}
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition-all ${
                          errors.seconds
                            ? "border-red-500 bg-red-50"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <textarea
                      name="description"
                      value={formSpecialData.description}
                      onChange={handleInputChange}
                  ...
                  <div className="space-y-4">
                    {editingId && !formSpecialData.mainPhoto ? (
                        ...
                        <div className="overflow-hidden rounded-lg border-2 border-dashed border-gray-300 aspect-video">
                          <img
                            src={`${baseUrl}/api/v1/file/getFile/${
                              specialData.find((c) => c.id === editingId)
                                ?.mainPhoto.id
                            }`}
                            alt="Asosiy rasm"
                ...
                <div className="flex items-center">
                  <label className="inline-flex items-center cursor-pointer">
                    <input
                      ...
                      id="status"
                      name="status"
                      checked={formSpecialData.status}
                      onChange={(e) =>
                        setFormSpecialData((prev) => ({
                          ...prev,
